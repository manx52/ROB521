...
class OccupancyGripMap:
    ...
    def scan_cb(self, scan_msg):
        ...
        x_start = ((odom_map[0]-self.map_msg.info.origin.position.x-MAP_DIM[0]/2)/CELL_SIZE).astype(int)
        y_start = ((odom_map[1]-self.map_msg.info.origin.position.y-MAP_DIM[1]/2)/CELL_SIZE).astype(int)
        for i in range(len(scan_msg.ranges)):
            if scan_msg.ranges[i] == np.inf:
                continue
            range_mes = scan_msg.ranges[i]/CELL_SIZE # Convert to pixels

            #Increments by 1 degree each time
            angle = odom_map[2]+i*scan_msg.angle_increment #Get angle + robot_heading
            map, log_odds = self.ray_trace_update(self.np_map, self.log_odds, x_start, y_start, angle, range_mes)
        self.np_map = map.copy()
        self.log_odds = log_odds.copy()
    ...
    def ray_trace_update(self, map, log_odds, x_start, y_start, angle, range_mes):
        ...
        x_end = x_start + np.rint(range_mes*np.cos(angle))
        y_end = y_start + np.rint(range_mes*np.sin(angle))
        
        rr,cc = ray_trace(int(y_start), int(x_start), int(y_end), int(x_end))
        trace_paths = np.vstack([rr, cc]).T
        
        for i in range(len(trace_paths)):
            if trace_paths[i][0] > map.shape[0]-1 or trace_paths[i][1] > map.shape[1]-1 or trace_paths[i][0] < 0 or trace_paths[i][1] < 1:
            # out of map
                break
            if i < trace_paths.shape[0]-NUM_PTS_OBSTACLE:
                log_odds[trace_paths[i][0],trace_paths[i][1]]=log_odds[trace_paths[i][0],trace_paths[i][1]]-BETA
            else:
                log_odds[trace_paths[i][0],trace_paths[i][1]]=log_odds[trace_paths[i][0],trace_paths[i][1]]+ALPHA
            prob = np.exp(log_odds[trace_paths[i][0],trace_paths[i][1]])/(1+np.exp(log_odds[trace_paths[i][0],trace_paths[i][1]]))
            map[trace_paths[i][0],trace_paths[i][1]] = np.rint(prob*100).astype(np.uint8)
