...
class PathPlanner:
    ...
    def sample_map_space(self):
        ...
        sample = np.zeros((2, 1))
        x_sample = np.random.uniform(low=self.bounds[0,0],high=self.bounds[0,1])
        y_sample = np.random.uniform(low=self.bounds[1,0], high=self.bounds[1,1])
        sample[0][0] = x_sample
        sample[0][1] = y_sample
        return sample
    ...
    def point_to_cell(self, point):
        ...
        new_point[0] = int((point[0] - self.bounds[0, 0]) / self.map_settings_dict["resolution"])
        new_point[1] = int((point[1] - self.bounds[1, 0]) / self.map_settings_dict["resolution"])
        return new_point
    ...
    def points_to_robot_circle(self, points):
        ...
        rr = []
        cc = []
        robot_radius = self.robot_radius / self.map_settings_dict["resolution"]
        for i, pt in enumerate(points):
            occ = self.point_to_cell(pt)
            temp_rr, temp_cc = circle(occ[0], occ[1], robot_radius)
            rr.append(temp_rr)
            cc.append(temp_cc)
        return rr, cc
...
def main():
    ...
    path_planner.points_to_robot_circle([[0, 0]])
